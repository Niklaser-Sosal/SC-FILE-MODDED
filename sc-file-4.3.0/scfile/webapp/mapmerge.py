...
def _ensure_scmapmerge_importable() -> None:
    ...
    env = os.getenv("SCMAPMERGE_DIR") or os.getenv("SCFILE_SCMAPMERGE_DIR")
    if env:
        p = Path(env)
        if (p / "scmapmerge").exists():
            sys.path.insert(0, str(p))
            return

    # Running from a monorepo: search upwards for a sibling `sc-mapmerge-*/` folder.
    # (Windows launcher keeps sc-file-* and sc-mapmerge-* next to each other.)
    here = Path(__file__).resolve()
    for i, parent in enumerate(here.parents):
        if i > 8:
            break

        direct = parent / "sc-mapmerge"
        if (direct / "scmapmerge").exists():
            sys.path.insert(0, str(direct))
            return

        candidates = sorted(parent.glob("sc-mapmerge-*"), reverse=True)
        for c in candidates:
