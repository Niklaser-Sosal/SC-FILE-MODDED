...
@click.option(
    "--web",
    help="Open sc-file Web UI (local server + window).",
    is_flag=True,
)
@click.option(
    "--no-window",
    help="With --web: open in browser instead of desktop window.",
    is_flag=True,
)
@click.option(
    "-O",
...
def scfile(
    ...
    if web:
        try:
            ...
            run(window=not no_window)
        except Exception as err:
            print(L.ERROR, f"Unable to start Web UI: {err}")
        return
